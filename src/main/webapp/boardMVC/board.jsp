		...
		<div class="col-md-8">
			...
			<div class="list-group">
				<c:forEach var="i" items="${boardList}" varStatus="cnt">
					<a href="boardInfo.do?no=${i.no}" class="list-group-item">
						<div style="float: left;">
							<c:forEach var="j" begin="1" end="${i.lev}">
				            	<img src="img/comment.jfif" alt="reply" height="50px">			
							</c:forEach>
							<c:if test="${i.fileName  ne null}">
								<img src="${pageContext.request.contextPath}/resources/upload/${i.fileName}" alt="No Image" height="50px">	
							</c:if>
						</div>
						<div class="board-title">
							<span class="board-category">[Thanks.com]</span>
							<span>${i.title} </span>  
							<span class="badge">"${i.no ne i.ref ? 'Reply' : 'New'}"</span>
						</div>
						<div class="board-meta" style="font-weight: 400; font-size: 1.2rem; color: #404040">
							<p>
								<i class="glyphicon glyphicon-user"></i> ${i.author}
								<i class="glyphicon glyphicon-comment"></i> ${i.ref}
								<i class="glyphicon glyphicon-ok"></i> ${i.hit}
